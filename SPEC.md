...
- **Staff/volunteer** — logs in, processes returned passports, can look up and correct past entries, runs reports. Expect up to **30 volunteers** working concurrently during the intake window. In practice, staff are assigned to Django groups matching their role (e.g. "Passport Logger" — view-only in the admin, all real data entry via the intake form which isn't gated by admin permissions at all; "Venue Admins" — manage the cafe list) with ordinary Django group permissions.
...
- **Admin** — a staff role with additional permissions to manage the cafe list, seasons, staff accounts, and data retention/purge — via a group with the relevant permissions, or (for Bearer records specifically, which are locked to superusers regardless of group permissions — see §5.2) full superuser status.
...
**Phone as the access key.** Searching by name only confirms a match exists (name shown, nothing else) and prompts for the bearer's phone number; only a correct phone number reveals their contact details and opens their submission. Searching directly by phone (any common format — normalized before matching) grants access immediately, no separate challenge. This stops a volunteer casually browsing another bearer's contact details/raffle status by name alone — genuine access requires actually having the phone number, which in practice means having the physical passport or the bearer present. The `Bearer` admin list (direct browsing of all bearers) is locked to superusers specifically — not just any Admin-role group — for the same reason; regular staff only ever reach bearer records through this gated search.
